...
(define (record-error e) (set! errs (cons (list cur-section e) errs)))
...
(SECTION 4 2 1)
(test 'greater 'cond (cond
                      ((> 3 2) 'greater)
                      ((< 3 2) 'less)))

(test 'equal 'cond (cond
                    ((> 3 3) 'greater)
                    ((< 3 3) 'less)
                    (else 'equal)))
...
(test '(list a (quote a)) 'quasiquote
      (let ((name 'a))
        `(list ,name ',name)))
...
(test 99 'internal-define
      (letrec ((foo (lambda (arg)
                      (or arg (and (procedure? foo)(foo 99))))))
        (define bar (foo #f))
        (foo #f)))

(test 77 'internal-define
      (letrec ((foo 77)
               (bar #f)
               (retfoo (lambda () foo)))
        (define baz (retfoo))
...
(define (test-inexact-printing)
  (let ((f0.0 (string->number "0.0"))
    ...
    (test #t 'mult-float-print-test (mult-float-print-test
				     (string->number "2.7182818284590451"))))) ;end test-inexact-printing
...
(let ((have-inexacts?
      ...
      (have-bignums?
       (let ((n (string->number "1427247692705959881058285969449495136382746625")))
         (and n (exact? n)))))
...
(newline)(display "(test-cont)")(newline)
